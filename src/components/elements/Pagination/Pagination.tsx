...
const Pagination: FC<Props> = ({ options, type = EnumColorStyle.dark }) => {
    ...
    return (
        endPage !== 1 && (
            <div className='relative flex gap-2.5 w-fit m-auto'>
                <button
                    type='button'
                    disabled={currentPage === 1}
                    onClick={() => setPage(`${currentPage - 1}`)}
                    className={cn(
                        'group flex items-center justify-center size-10 md:size-12 rounded-lg transition-all duration-200 active:scale-95 hover:bg-violet-600',
                ...
                <div className={`flex items-center height-btn px-5 rounded-lg ${paginationClasses[type]}`}>
